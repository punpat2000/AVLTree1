public class AVLTree {
	...
	public void makeAVL() throws Exception {
		//code this method
		this.makeAVL(this.root);
	}
	
	private void makeAVL(AVLNode n) {
		if (n == null)
			return;
		if (Math.abs(AVLNode.tiltDegree(n)) > 1)
			this.rebalance(n);
		this.makeAVL(n.left);
		this.makeAVL(n.right);
	}
	...
	private boolean isAVL(AVLNode n) {
		return n == null ? true : Math.abs(AVLNode.tiltDegree(n)) > 1 ? false : this.isAVL(n.left) && this.isAVL(n.right);
	}
